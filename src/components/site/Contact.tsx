...
function ContactItem({ icon: Icon, title, content, link, delay = 0 }: { icon: any, title: string, content: string, link?: string, delay?: number }) {
  ...
  return (
    ...
    >
      <div className="flex h-14 w-14 shrink-0 items-center justify-center rounded-xl bg-velvet text-gold shadow-xl border border-white/5">
      ...
      <div>
        ...
        <Wrapper 
          ...
          className={`mt-2 block font-serif text-2xl text-velvet ${link ? 'hover:text-gold transition-colors underline decoration-gold/20 underline-offset-8' : ''}`}
        >
          {content}
        </Wrapper>
      </div>
