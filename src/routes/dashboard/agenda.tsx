...
function DashboardAgenda() {
  ...
  const events = [
    { id: 1, title: 'Audiência de Instrução - Caso Rural', time: '14:30', date: '25', type: 'Judicial', color: 'bg-red-500' },
    { id: 2, title: 'Reunião com Perito Agrônomo', time: '10:00', date: '26', type: 'Reunião', color: 'bg-gold' },
    { id: 3, title: 'Prazo: Contestação Fazenda S. José', time: '18:00', date: '28', type: 'Prazo', color: 'bg-blue-500' },
  ]

  const integrations = [
    { name: 'Google Calendar', icon: 'https://upload.wikimedia.org/wikipedia/commons/a/a5/Google_Calendar_icon_%282020%29.svg', connected: true },
    { name: 'Outlook / Microsoft', icon: 'https://upload.wikimedia.org/wikipedia/commons/d/df/Microsoft_Office_Outlook_%282018%E2%80%93present%29.svg', connected: false },
    { name: 'Apple Calendar', icon: 'https://upload.wikimedia.org/wikipedia/commons/b/b8/Ios-calendar-icon.png', connected: false },
  ]
          ...
          <div className="bg-white rounded-3xl border border-stone-100 shadow-sm overflow-hidden">
            ...
            <div className="p-3 md:p-8 border-b border-stone-50 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
              <div className="flex items-center justify-between sm:justify-start gap-4">
                ...
                <div className="flex items-center gap-1">
                  <button className="p-1.5 hover:bg-stone-50 rounded-lg transition-colors"><ChevronLeft className="h-4 w-4 text-stone-400" /></button>
                  <button className="p-1.5 hover:bg-stone-50 rounded-lg transition-colors"><ChevronRight className="h-4 w-4 text-stone-400" /></button>
                </div>
              </div>
              <div className="flex bg-stone-50 p-1 rounded-xl w-full sm:w-fit overflow-hidden">
                <button className="flex-1 sm:flex-none px-2 md:px-4 py-1.5 rounded-lg text-[8px] md:text-[10px] font-black uppercase tracking-widest bg-white text-velvet shadow-sm">Mês</button>
                <button className="flex-1 sm:flex-none px-2 md:px-4 py-1.5 rounded-lg text-[8px] md:text-[10px] font-black uppercase tracking-widest text-stone-400 hover:text-velvet">Semana</button>
                <button className="flex-1 sm:flex-none px-2 md:px-4 py-1.5 rounded-lg text-[8px] md:text-[10px] font-black uppercase tracking-widest text-stone-400 hover:text-velvet">Dia</button>
              </div>
            </div>
              ...
              <div className="min-w-[320px]">
                <div className="grid grid-cols-7 border-b border-stone-50">
                  {['D', 'S', 'T', 'Q', 'Q', 'S', 'S'].map(day => (
                    <div key={day} className="py-3 text-center text-[9px] font-black uppercase tracking-widest text-stone-400">
                      {day}
                    </div>
                  ))}
                </div>
                    ...
                    return (
                      <div key={i} className={`border-r border-b border-stone-50 p-1.5 md:p-3 group hover:bg-stone-50/50 transition-colors cursor-pointer relative ${i % 7 === 6 ? 'border-r-0' : ''}`}>
                        <div className="flex flex-col h-full">
                          <span className={`text-[10px] md:text-xs font-bold mb-1 ${day > 0 && day <= 31 ? (isToday ? 'text-white bg-gold h-5 w-5 md:h-6 md:w-6 flex items-center justify-center rounded-full' : 'text-stone-400 group-hover:text-velvet') : 'text-stone-100'}`}>
                            {day > 0 && day <= 31 ? day : ''}
                          </span>
                          {day > 0 && day <= 31 && hasEvent && (
                            <div className="flex-1 flex flex-col justify-end">
                              {events.filter(e => e.date === String(day)).map(event => (
                                <div key={event.id} className="group/event relative">
                                  <div className={`h-1 md:h-1.5 w-full rounded-full ${event.color} mb-0.5 opacity-60 group-hover:opacity-100 transition-opacity`} />
            ...
            <div className="p-4 border-t border-stone-50 bg-stone-50/20 flex flex-wrap gap-4 items-center justify-center">
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-[9px] font-black text-stone-500 uppercase tracking-widest">Judicial</span>
              </div>
              <div className="flex items-center gap-1.5">
                <div className="h-2 w-2 rounded-full bg-gold" />
                <span className="text-[9px] font-black text-stone-500 uppercase tracking-widest">Reunião</span>
              </div>
              <div className="flex items-center gap-1.5">
                <div className="h-2 w-2 rounded-full bg-blue-500" />
                <span className="text-[9px] font-black text-stone-500 uppercase tracking-widest">Prazo</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="xl:col-span-4 space-y-8">
              ...
              <div className="space-y-4">
                {integrations.map((app) => (
                  <div key={app.name} className="flex items-center justify-between p-4 rounded-2xl bg-white/5 border border-white/10 hover:bg-white/10 transition-all cursor-pointer group">
                    ...
                    {app.connected ? (
                      <span className="text-[8px] bg-green-500/20 text-green-400 font-black uppercase tracking-widest px-2 py-1 rounded-md">Ativo</span>
                    ) : (
                      <span className="text-[8px] text-white/40 group-hover:text-gold transition-colors font-black uppercase tracking-widest">Conectar</span>
                    )}
                  </div>
                ))}
              </div>
          ...
          <div className="bg-white rounded-3xl border border-stone-100 shadow-sm overflow-hidden">
            <div className="p-6 border-b border-stone-50">
              <h3 className="font-serif text-lg text-velvet">Próximos <span className="italic text-gold">Compromissos</span></h3>
            </div>
            <div className="p-4 space-y-2">
              {events.map((event) => (
                <div key={event.id} className="p-4 rounded-2xl hover:bg-stone-50 transition-colors border border-transparent hover:border-stone-100 group cursor-pointer">
                  <div className="flex items-start justify-between">
                    <div className="flex gap-4">
                      <div className={`h-10 w-10 shrink-0 rounded-xl ${event.color} flex flex-col items-center justify-center text-white`}>
                        <span className="text-[10px] font-bold uppercase leading-none opacity-60">Out</span>
                        <span className="text-sm font-black leading-none mt-1">{event.date}</span>
                      </div>
                      <div>
                        <p className="text-xs font-bold text-velvet group-hover:text-gold transition-colors">{event.title}</p>
