...
function DashboardCases() {
  const cases = [
    { id: '0012345-67.2026.8.17.0001', client: 'Fazenda Boa Esperança', subject: 'Reintegração de Posse', status: 'Em Instrução', date: '12 Out 2026', files: 12 },
    { id: '0098765-43.2026.8.17.0001', client: 'João Batista da Silva', subject: 'Inventário e Partilha', status: 'Aguardando Sentença', date: '05 Out 2026', files: 24 },
    { id: '0044556-89.2026.8.17.0001', client: 'Cooperativa Agro Rural', subject: 'Defesa Ambiental / CPR', status: 'Petição Inicial', date: '20 Out 2026', files: 5 },
  ]
    ...
    <div className="space-y-6 md:space-y-8">
      <header className="flex flex-col gap-5 md:flex-row md:items-center md:justify-between">
        <div className="max-w-xs md:max-w-none">
          ...
          <p className="text-stone-500 text-[10px] sm:text-[11px] md:text-sm mt-1 uppercase tracking-wider font-medium opacity-70">Controle de ativos jurídicos digitais</p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4 md:gap-6">
        <div className="bg-white p-6 rounded-3xl border border-stone-100 shadow-sm">
          <p className="text-[10px] uppercase tracking-widest text-stone-500 font-bold">Total de Processos</p>
          <p className="text-2xl font-sans font-black text-velvet mt-1">128</p>
        </div>
        <div className="bg-white p-6 rounded-3xl border border-stone-100 shadow-sm">
          <p className="text-[10px] uppercase tracking-widest text-stone-500 font-bold">Petições Pendentes</p>
        ...
        <div className="bg-white p-6 rounded-3xl border border-stone-100 shadow-sm sm:col-span-2 lg:col-span-1">
          <p className="text-[10px] uppercase tracking-widest text-stone-500 font-bold">Documentos Armazenados</p>
            ...
            >
              <div className="flex flex-col md:flex-row md:items-center justify-between mb-6 gap-4">
                <div className="flex flex-wrap items-center gap-2">
                  <span className="text-[9px] font-sans font-bold bg-stone-100 text-stone-500 px-3 py-1 rounded-full uppercase tracking-wider">{c.id}</span>
                  <span className="text-[9px] font-sans font-bold bg-gold/10 text-gold px-3 py-1 rounded-full uppercase tracking-wider">{c.status}</span>
                </div>
              ...
              <div className="grid grid-cols-1 xl:grid-cols-2 gap-6 md:gap-8">
                <div>
                  <h4 className="text-lg md:text-xl font-serif text-velvet group-hover:text-gold transition-colors leading-tight">{c.subject}</h4>
                  <p className="text-xs font-bold text-stone-500 mt-2 uppercase tracking-widest">{c.client}</p>
                </div>
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-end gap-6 md:gap-12">
                  <div className="flex sm:flex-col items-center sm:items-end justify-between w-full sm:w-auto gap-2">
                    <p className="text-[9px] text-stone-400 font-bold uppercase tracking-widest">Movimentação</p>
                  ...
                  <div className="flex sm:flex-col items-center sm:items-end justify-between w-full sm:w-auto gap-2">
                    <p className="text-[9px] text-stone-400 font-bold uppercase tracking-widest">Arquivos</p>
                    <div className="flex items-center gap-2 text-stone-600">
